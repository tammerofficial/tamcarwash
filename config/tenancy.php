...
return [
    ...
    'reserved_paths' => array_values(array_unique(array_filter(array_map(
        'strtolower',
        array_map('trim', explode(',', env('TENANCY_RESERVED_PATHS', 'api,login,register,sanctum,up,landlord,build,storage,dashboard,settings,appearance,branches,customers,vehicles,services,pricing,booking,queue,tv,cashier,worker,orders,invoices,tax-reports')))
    )))),
