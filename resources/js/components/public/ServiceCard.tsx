...
export function ServiceCard({ service, currency }: ServiceCardProps) {
    return (
        <Card className="group relative overflow-hidden rounded-2xl border border-gray-100 bg-white transition-all duration-500 hover:border-teal-600/10 hover:shadow-xl hover:-translate-y-1">
            <div className="relative h-56 overflow-hidden">
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-[#004d4d]/90 via-[#004d4d]/20 to-transparent opacity-60 group-hover:opacity-80 transition-opacity" />
                
                <div className="absolute top-5 left-5">
                    <Badge className="bg-white/95 backdrop-blur-md text-teal-900 border-none font-bold px-3 py-1 text-[10px] uppercase tracking-wider">
                        {service.category?.name || 'غسيل'}
                    </Badge>
                </div>

                <div className="absolute bottom-5 right-6 left-6">
                    <h3 className="text-xl font-bold text-white mb-1 group-hover:text-teal-400 transition-colors">{service.name_ar || service.name}</h3>
            ...
            <div className="p-6">
                ...
                <div className="flex items-center justify-between pt-5 border-t border-gray-50">
                    <div>
                        ...
                        <p className="text-2xl font-bold text-[#006666]">
                    ...
                    <Button 
                        size="icon" 
                        variant="ghost"
                        className="h-12 w-12 rounded-xl bg-teal-50/50 text-teal-600 hover:bg-teal-600 hover:text-white transition-all shadow-sm group-hover:shadow-md"
                    >
